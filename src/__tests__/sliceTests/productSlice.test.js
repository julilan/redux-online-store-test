...
// We need to use thunk middleware to test async actions
...
describe('fetchProducts', () => {
  beforeEach(() => {
    jest.setTimeout(10000); // Increase timeout for slow API calls
  });
  ...
  it('Product 1 has specifi data to our test case', async () => {
    const store = mockStore({ products: [] });
    await store.dispatch(fetchProducts());
